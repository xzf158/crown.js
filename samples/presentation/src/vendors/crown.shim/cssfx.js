 ...
 */
/* jshint ignore:start */
define(['jquery', 'crown.browser'], function ($, browser) {
    /*
     * cssFx process:
     * 
    ...
    fx.insertCSS = function (output) {
        for (var x = 0; x < output.length; x++) {
            ...
            css_fx_output.setAttribute('data-created-by', 'cssfx');
                ...
                switch (property) {
                    ...
                    case __background:
                        ...
                        if (value.indexOf(lg) >= 0) {
                            ...
                            var hex = padLeft(Math.floor(+(str_combo(cA[3])) * 255).toString(16), 2) + rgb2hex(+str_combo(cA[0]), +str_combo(cA[1]), +str_combo(cA[2]));
    ...
    function padLeft(str, length, character) {
        return new Array(length - str.length + 1).join(character || '0') + str;
    }
    function init() {
        $('link[rel=stylesheet]').each(function () {
            if (typeof (this) === "object") {
                $.get($(this).attr('href')).done(function (css) {
                    fx.insertCSS(fx.processCSS([css]))
                });
            }
        });
        var css_files = [];
