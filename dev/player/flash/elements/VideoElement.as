package elements
{
	...
	public class VideoElement extends Sprite implements IMediaElement 
	{
		...
		public function getIsEnded ():Boolean{
			return _isEnded;
		}
		...
		public function getCurrentProgress():Number {
			if(_stream != null) {
		...
		public function VideoElement(element:FlashMediaElement, autoplay:Boolean, preload:String, timerRate:Number, startVolume:Number, startMuted:Boolean, streamer:String) 
		{
			_element = element;
			_autoplay = autoplay;
			_preload = preload;
		...
		private function netStatusHandler(event:NetStatusEvent):void {
			_element.sendLog("netStatus", event.info.code);
			switch (event.info.code) {
				...
				case "NetStream.Pause.Notify":
					_isPaused = true;
					_element.sendLog("NetStream.Pause.Notify");
				...
				case "NetStream.Play.Stop":
					...
					_isPaused = false;
					_timer.stop();
					_element.sendLog("NetStream.Play.Stop");
		...
		public function play():void {
			_element.sendLog('_hasStartedPlaying:' + _hasStartedPlaying + ' _isConnected:' + _isConnected);
			_isEnded = false;
		...
		public function setIsMuted(muted:Boolean):void {
			if (_isMuted === muted)
				return;
		...
		public function setIsPaused(paused:Boolean):void {
			if (_isPaused === paused)
				return;
		...
		private function parseRTMP(url:String) {
			...
			_element.sendLog("parseRTMP - server: " + rtmpInfo.server + " stream: " + rtmpInfo.stream);
			return rtmpInfo;
		}
	}
}
